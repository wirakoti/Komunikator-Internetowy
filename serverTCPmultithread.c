...
int login_exists(const char *login) {
    ...
    if (rc == SQLITE_ROW) {#include <stdio.h>
#include <stdlib.h>
#include <sys/socket.h>
#include <netinet/in.h>
#include <string.h>
...
void *socketThread(void *arg) {
    ...
    // Extract the username from the message (after "OK ")
    char username[256];
    sscanf(client_message, "OK %s", username);  // Extract the username from the message
    ...
    // Bind the username to the SQL query (for both sender and receiver)
    sqlite3_bind_text(stmt, 1, username, -1, SQLITE_STATIC);
    sqlite3_bind_text(stmt, 2, username, -1, SQLITE_STATIC);

    // Loop through the results and append the usernames to the friends list
    int first_friend = 1;  // Used to manage leading spaces
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        const char *sender = (const char *)sqlite3_column_text(stmt, 0);
        const char *receiver = (const char *)sqlite3_column_text(stmt, 1);

        // Add both the sender and receiver to the friends list
        if (strcmp(sender, username) == 0) {
            if (!first_friend) strcat(friendsList, " ");
    ...
    char *receiver = strtok(client_message + 11, " ");  // Get the second part after ADD_FRIEND

    if (receiver) {
        // Step 1: Check if the reverse relation exists (receiver=username1, sender=username2)
        sqlite3_stmt *stmt;
        ...
        if (rc == SQLITE_ROW) {
            ...
            // Step 3: Insert a new friend request (status = 1)
...
void *socketThread(void *arg) {
    ...
    if (receiver) {
        ...
        rc = sqlite3_step(stmt);
        if (rc == SQLITE_ROW) {
            // update friendship (exists if status=2)
            const char *update_sql = "UPDATE friends SET status = 2 WHERE sender = ? AND receiver = ?;";
            rc = sqlite3_prepare_v2(db, update_sql, -1, &stmt, 0);
            if (rc != SQLITE_OK) {
                fprintf(stderr, "SQL error: %s\n", sqlite3_errmsg(db));
            ...
            // sending new friend request (status = 1)
            const char *insert_sql = "INSERT INTO friends (sender, receiver, status) VALUES (?, ?, 1);";
            rc = sqlite3_prepare_v2(db, insert_sql, -1, &stmt, 0);
            if (rc != SQLITE_OK) {
                fprintf(stderr, "SQL error: %s\n", sqlite3_errmsg(db));
